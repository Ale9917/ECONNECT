...
function iniciarSesion() {
    ...
    if (index !== -1) {
        usuarios[index].ultimoAcceso = usuario.ultimoAcceso;
        usuarios[index].sesionActiva = true;
        localStorage.setItem("usuarios", JSON.stringify(usuarios));
    }
...
function completarDesafio(desafioId) {
    ...
    setTimeout(() => {
        ...
        if (!usuario.desafiosCompletados.includes(desafioId)) {
            ...
            mostrarNotificacion(`¡Felicidades! Has completado el desafío "${titulo}" y has ganado ${puntosExtra} puntos.`);
            
            // Añadir animación de confeti
            mostrarConfeti();
            
            // Mostrar el quiz asociado al desafío
            setTimeout(() => {
                mostrarQuiz(desafioId, titulo);
            }, 1500);
